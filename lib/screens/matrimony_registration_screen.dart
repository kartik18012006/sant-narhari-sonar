...
class _MatrimonyRegistrationScreenState extends State<MatrimonyRegistrationScreen> {
  ...
  Future<String?> _uploadPhoto(String photoType) async {
    ...
    try {
      ...
      final path = FirebaseStorageService.instance.uniquePath('matrimony/${user.uid}/$photoType', extension: 'jpg');
  ...
  Future<String?> _uploadDocument(String documentType) async {
    ...
    try {
      ...
      final path = FirebaseStorageService.instance.uniquePath('matrimony/${user.uid}/documents/$documentType', extension: 'jpg');
      final url = await FirebaseStorageService.instance.uploadImage(bytes: bytes, path: path);
      if (!mounted) return null;
      if (url != null) {
        ScaffoldMessenger.of(context).showSnackBar(
